...
from dbt_server.flags import DBT_WORKING_DIR
...
def includes_protocol(path: str) -> bool:
    return len(path.split("://")) > 1


def protocol() -> str:
    path = DBT_WORKING_DIR.get()
    if includes_protocol(path):
        return path.split("://")[0]
    else:
        return "file"


filesystem: fsspec.AbstractFileSystem = fsspec.filesystem(protocol())
...
def get_working_dir():
    ...
    files."""
    return abspath(DBT_WORKING_DIR.get())
...
def get_target_path():
    ...
    return abspath(os.environ.get("DBT_TARGET_PATH", DEFAULT_TARGET_DIR))
...
def get_root_path(state_id: str = None, project_path: str = None):
    ...
    if project_path is not None:
        return abspath(project_path)
    if state_id is None:
        return None
    working_dir = get_working_dir()
    return abspath(os.path.join(working_dir, f"state-{state_id}"))
...
def get_db_path():
    ...
    not existed."""
    working_dir = get_working_dir()
    path = abspath(os.path.join(working_dir, DATABASE_FILE_NAME))
...
def get_latest_state_id(state_id: str):
    ...
    """
    if not state_id:
        path = abspath(get_latest_state_file_path())
...
def get_latest_project_path():
    ...
    path = abspath(get_latest_project_path_file_path())
    if not filesystem.exists(path):
        return None
    with filesystem.open(path, "r") as latest_path_file:
...
def upload(src: str, dest: str):
    filesystem.put(src, dest + filesystem.sep, recursive=True)


def download(src: str, dest: str):
    filesystem.get(src + filesystem.sep, dest, recursive=True)
